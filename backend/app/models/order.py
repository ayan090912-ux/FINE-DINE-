...
class OrderItem(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    selected_addons: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True) # JSON list of selected addon objects [{name, price, qty}]
...
class Feedback(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    overall_rating: Mapped[int] = mapped_column(Integer, nullable=False) # 1 to 5 stars
